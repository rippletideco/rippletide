...
Each agent ID maps to an **isolated knowledge graph**. Memories stored under one agent ID are invisible to another. You can:
...
If you need to run the MCP server yourself (e.g. for data privacy or custom backends), you can configure it via environment variables or CLI flags. **CLI flags take priority.**
...
**HTTP** -- The MCP server runs as a standalone service and the AI client connects over HTTP. Use this for remote/shared deployments.
